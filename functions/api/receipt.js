...
const schema={type:'object',additionalProperties:false,properties:{currency:{type:['string','null']},items:{type:'array',maxItems:150,items:{type:'object',additionalProperties:false,properties:{name:{type:'string'},price:moneySchema},required:['name','price']}},tax:moneySchema,service:moneySchema,discount:moneySchema,total:moneySchema,needsReview:{type:'boolean'}},required:['currency','items','tax','service','discount','total','needsReview']};
...
export async function onRequest({request,env}){
 ...
 if(message.includes('schema')||message.includes('responseformat')||message.includes('response format'))error='SCHEMA_CONFIG';
